...
# 圧電スピーカー（GPIO21）を鳴らす関数
def beep_buzzer():
    buzzer = machine.PWM(machine.Pin(21))
    buzzer.freq(1000)  # 音の高さを指定（1000Hz）
    buzzer.duty(512)   # 音の大きさを指定（0〜1023、512が最大音量）
    time.sleep(0.1)    # 0.1秒間鳴らす
    buzzer.duty(0)     # 音を止める
    buzzer.deinit()    # PWMを解放する
    ...
    while True:
        ...
        if distance < 0:
            ...
            if distance < 10:  # <--- 赤色LEDが点灯する距離はここを変える
                ...
                # 圧電スピーカーを鳴らすには以下の行のコメントアウトを解除
                #beep_buzzer()
                
            elif 10 <= distance <= 50:  # <--- 黄色と緑色はここを変える
