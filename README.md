...
- 距離が10 cm未満の場合: LEDを赤く点灯（近距離）。
- 距離が10 cmから50 cmの場合: LEDを黄色に点灯（中距離）。
- 距離が50 cmを超える場合: LEDを緑色に点灯（遠距離）。

LEDはループで点灯し、割り込み（例: CTRL-C）が入るまで続き、その時点で全てのLEDをオフにします。

## ハードウェア要件

- MicroPython対応のマイクロコントローラ。
- HC-SR04超音波センサ。
- IoT学習ボード(ESP32-C3M-TRY)。

## 各ファイルについて

- OPC_001.py: カスタマイズ済み完成品(ESP32-C3M-TRY用)
- OPC_custom_message.py: Pythonプロンプトに表示されるメッセージを変更するカスタムコード
- OPC_student_handouts_TRY.py: 体験授業を受講者が最初に開いているコード(ESP32-C3M-TRY用)
- README.md: 本文書
...
スクリプトの内容：
- **LED設定**: 10～12のピンを出力として設定。
- **センサ設定**: Trig（4）を出力、Echo（5）を入力として設定。
